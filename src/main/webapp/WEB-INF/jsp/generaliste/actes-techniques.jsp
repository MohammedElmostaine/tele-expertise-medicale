<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Actes Techniques - Généraliste</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.7.2/font/bootstrap-icons.css" rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        * { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; }
        @keyframes slideInUp {
            from { opacity: 0; transform: translateY(40px); }
            to { opacity: 1; transform: translateY(0); }
        }
        .animate-slide-up { animation: slideInUp 0.8s ease-out; }
        .modal { display: none; position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4); }
        .modal.show { display: flex; align-items: center; justify-content: center; }
    </style>
</head>
<body class="bg-gradient-to-br from-blue-50 via-cyan-50 to-slate-50 min-h-screen overflow-x-hidden">

<!-- Background Effects -->
<div class="fixed inset-0 -z-20 bg-gradient-to-br from-blue-100/50 via-cyan-100/30 to-slate-100/40"></div>
<div class="fixed inset-0 -z-10 opacity-30" style="background-image: radial-gradient(circle at 20% 80%, rgba(37, 99, 235, 0.08) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(8, 145, 178, 0.08) 0%, transparent 50%);"></div>

<!-- Navigation -->
<nav class="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-xl border-b border-white/20 shadow-xl">
    <div class="container mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
            <a href="${pageContext.request.contextPath}/generaliste/consultation" class="flex items-center group">
                <div class="w-12 h-12 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-2xl flex items-center justify-center mr-3 shadow-lg shadow-blue-500/30 transition-all duration-300 group-hover:rotate-6 group-hover:scale-110">
                    <i class="bi bi-heart-pulse text-white text-2xl"></i>
                </div>
                <span class="text-2xl font-extrabold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">TeleExpertise</span>
            </a>
            <div class="flex items-center space-x-4">
                <div class="flex items-center bg-white/90 backdrop-blur-md border border-white/20 rounded-2xl px-4 py-2 shadow-lg">
                    <div class="w-9 h-9 bg-gradient-to-br from-cyan-600 to-cyan-500 rounded-xl flex items-center justify-center mr-3 shadow-md shadow-cyan-500/30">
                        <i class="bi bi-person-badge text-white"></i>
                    </div>
                    <div class="flex flex-col mr-3">
                        <span class="text-sm font-semibold text-slate-800">Dr. ${user.prenom} ${user.nom}</span>
                        <span class="text-xs font-medium text-white bg-gradient-to-r from-blue-600 to-blue-500 px-2 py-0.5 rounded-lg shadow-sm">Généraliste</span>
                    </div>
                </div>
                <a class="px-4 py-2 text-sm font-semibold text-red-600 border-2 border-red-600 rounded-xl hover:bg-gradient-to-r hover:from-red-600 hover:to-red-500 hover:text-white hover:border-transparent transition-all duration-300 hover:-translate-y-1" href="${pageContext.request.contextPath}/auth/logout">
                    <i class="bi bi-box-arrow-right mr-2"></i>Déconnexion
                </a>
            </div>
        </div>
    </div>
</nav>

<!-- Hero Section -->
<div class="mt-20 bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-12 relative overflow-hidden">
    <div class="absolute inset-0 opacity-30" style="background-image: url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%23ffffff\' fill-opacity=\'0.1\'%3E%3Ccircle cx=\'30\' cy=\'30\' r=\'2\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E');"></div>
    <div class="container mx-auto px-4 relative z-10">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-4xl font-bold mb-2 animate-slide-up">
                    <i class="bi bi-tools mr-3"></i>Gestion des Actes Techniques
                </h1>
            </div>
            <div class="flex space-x-3">
                <button onclick="document.getElementById('ajouterActeModal').classList.add('show')" class="px-6 py-3 bg-white text-blue-600 rounded-xl font-semibold hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
                    <i class="bi bi-plus-circle mr-2"></i>Ajouter Acte
                </button>
                <a href="consultation?action=details&id=${consultation.id}" class="px-6 py-3 bg-white/20 backdrop-blur-md border border-white/30 rounded-xl font-semibold hover:bg-white/30 transition-all duration-300">
                    <i class="bi bi-arrow-left mr-2"></i>Retour
                </a>
            </div>
        </div>
    </div>
</div>

<div class="container mx-auto px-4 py-8">
    <!-- Informations de la consultation -->
    <div class="bg-white/95 backdrop-blur-md border border-white/20 rounded-3xl shadow-xl mb-8 animate-slide-up overflow-hidden">
        <div class="bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-6 py-4">
            <h5 class="text-xl font-bold mb-0">Consultation en cours</h5>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-slate-600 mb-2"><strong class="text-slate-800">Patient :</strong> ${consultation.patient.prenom} ${consultation.patient.nom}</p>
                    <p class="text-slate-600"><strong class="text-slate-800">Motif :</strong> ${consultation.motif}</p>
                </div>
                <div>
                    <p class="text-slate-600 mb-2">
                        <strong class="text-slate-800">Date :</strong>
                        ${consultation.dateConsultation.dayOfMonth}/${consultation.dateConsultation.monthValue}/${consultation.dateConsultation.year}
                        ${consultation.dateConsultation.hour}h${consultation.dateConsultation.minute < 10 ? '0' : ''}${consultation.dateConsultation.minute}
                    </p>
                    <p class="text-slate-600"><strong class="text-slate-800">Coût de base :</strong> <span class="text-green-600 font-bold">${consultation.coutConsultation} DH</span></p>
                </div>
            </div>
        </div>
    </div>

    <!-- Liste des actes techniques -->
    <div class="bg-white/95 backdrop-blur-md border border-white/20 rounded-3xl shadow-xl mb-8 animate-slide-up overflow-hidden">
        <div class="bg-gradient-to-r from-amber-500 to-yellow-500 text-white px-6 py-4 flex justify-between items-center">
            <h5 class="text-xl font-bold mb-0"><i class="bi bi-list-ul mr-3"></i>Actes Techniques Réalisés</h5>
            <c:set var="totalActes" value="0"/>
            <c:forEach var="acte" items="${actesTechniques}">
                <c:set var="totalActes" value="${totalActes + acte.cout}"/>
            </c:forEach>
            <span class="px-4 py-2 bg-white/20 backdrop-blur-md rounded-lg font-semibold">Total: ${totalActes} DH</span>
        </div>
        <div class="p-6">
            <c:choose>
                <c:when test="${empty actesTechniques}">
                    <div class="text-center py-12">
                        <i class="bi bi-clipboard-x text-6xl text-slate-300 mb-4"></i>
                        <h5 class="text-xl font-bold text-slate-600 mb-2">Aucun acte technique réalisé</h5>
                        <p class="text-slate-500">Cliquez sur "Ajouter Acte" pour commencer</p>
                    </div>
                </c:when>
                <c:otherwise>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead>
                                <tr class="border-b-2 border-slate-200">
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-700">Type d'Acte</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-700">Description</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-700">Coût</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-700">Date</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-700">Réalisé par</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-700">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach var="acte" items="${actesTechniques}">
                                    <tr class="border-b border-slate-100 hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-cyan-50/50 transition-all">
                                        <td class="px-4 py-4">
                                            <strong class="text-slate-800">${acte.type.libelle}</strong>
                                            <br><small class="text-slate-500">${acte.type.description}</small>
                                        </td>
                                        <td class="px-4 py-4 text-slate-600">
                                            <c:choose>
                                                <c:when test="${not empty acte.description}">
                                                    ${acte.description}
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="text-slate-400 italic">Aucune description</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td class="px-4 py-4">
                                            <span class="text-green-600 font-bold text-lg">${acte.cout} DH</span>
                                        </td>
                                        <td class="px-4 py-4 text-slate-600">
                                            ${acte.dateRealisation.dayOfMonth}/${acte.dateRealisation.monthValue}/${acte.dateRealisation.year}
                                            <br><small>${acte.dateRealisation.hour}h${acte.dateRealisation.minute < 10 ? '0' : ''}${acte.dateRealisation.minute}</small>
                                        </td>
                                        <td class="px-4 py-4 text-slate-600">
                                            Dr. ${acte.realisePar.prenom} ${acte.realisePar.nom}
                                        </td>
                                        <td class="px-4 py-4">
                                            <c:if test="${consultation.statut != 'TERMINEE'}">
                                                <form method="post" action="consultation" style="display: inline;">
                                                    <input type="hidden" name="action" value="supprimer-acte">
                                                    <input type="hidden" name="acteId" value="${acte.id}">
                                                    <input type="hidden" name="consultationId" value="${consultation.id}">
                                                    <button type="submit" class="px-3 py-2 bg-gradient-to-r from-red-600 to-red-500 text-white rounded-lg text-sm font-semibold hover:shadow-lg transition-all duration-300"
                                                            onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet acte ?')">
                                                        <i class="bi bi-trash"></i>
                                                    </button>
                                                </form>
                                            </c:if>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                            <tfoot>
                                <tr class="border-t-2 border-slate-200 bg-slate-50">
                                    <td colspan="2" class="px-4 py-3 font-bold text-slate-800">Total des actes techniques</td>
                                    <td class="px-4 py-3 font-bold text-green-600 text-lg">${totalActes} DH</td>
                                    <td colspan="3"></td>
                                </tr>
                                <tr class="bg-slate-50">
                                    <td colspan="2" class="px-4 py-3 font-bold text-slate-800">Coût consultation de base</td>
                                    <td class="px-4 py-3 font-bold text-blue-600 text-lg">${consultation.coutConsultation} DH</td>
                                    <td colspan="3"></td>
                                </tr>
                                <tr class="bg-gradient-to-r from-green-100 to-green-50 border-t-2 border-green-300">
                                    <td colspan="2" class="px-4 py-4 font-bold text-green-800 text-lg">TOTAL GÉNÉRAL</td>
                                    <td class="px-4 py-4 font-bold text-green-600 text-2xl">${consultation.coutTotal} DH</td>
                                    <td colspan="3"></td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
    </div>

    <!-- Résumé financier -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-gradient-to-br from-green-600 to-green-500 text-white rounded-3xl shadow-xl p-6 transition-all duration-300 hover:-translate-y-2">
            <h6 class="text-lg font-bold mb-4 flex items-center"><i class="bi bi-calculator mr-2"></i>Résumé Financier</h6>
            <div class="space-y-3">
                <div class="flex justify-between items-center pb-2 border-b border-white/20">
                    <span class="opacity-90">Consultation :</span>
                    <span class="font-bold">${consultation.coutConsultation} DH</span>
                </div>
                <div class="flex justify-between items-center pb-2 border-b border-white/20">
                    <span class="opacity-90">Actes techniques :</span>
                    <span class="font-bold">${totalActes} DH</span>
                </div>
                <div class="flex justify-between items-center pt-2 text-xl">
                    <strong>Total à payer :</strong>
                    <strong>${consultation.coutTotal} DH</strong>
                </div>
            </div>
        </div>
        <div class="bg-gradient-to-br from-blue-600 to-cyan-600 text-white rounded-3xl shadow-xl p-6 transition-all duration-300 hover:-translate-y-2">
            <h6 class="text-lg font-bold mb-4 flex items-center"><i class="bi bi-info-circle mr-2"></i>Informations</h6>
            <div class="space-y-2 opacity-90">
                <p class="flex items-center"><i class="bi bi-check-circle mr-2"></i>Coût de consultation fixe : 150 DH</p>
                <p class="flex items-center"><i class="bi bi-tools mr-2"></i>${actesTechniques.size()} acte(s) technique(s) réalisé(s)</p>
                <p class="flex items-center"><i class="bi bi-calendar mr-2"></i>Consultation du ${consultation.dateConsultation.dayOfMonth}/${consultation.dateConsultation.monthValue}/${consultation.dateConsultation.year}</p>
            </div>
        </div>
    </div>
</div>

<!-- Modal pour ajouter un acte technique -->
<div id="ajouterActeModal" class="modal">
    <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full mx-4 animate-slide-up overflow-hidden">
        <form action="consultation" method="post">
            <input type="hidden" name="action" value="ajouter-acte">
            <input type="hidden" name="consultationId" value="${consultation.id}">

            <div class="bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-6 py-4 flex justify-between items-center">
                <h5 class="text-xl font-bold"><i class="bi bi-plus-circle mr-2"></i>Ajouter un Acte Technique</h5>
                <button type="button" onclick="document.getElementById('ajouterActeModal').classList.remove('show')" class="text-white hover:text-gray-200">
                    <i class="bi bi-x-lg text-2xl"></i>
                </button>
            </div>
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="typeActe" class="block text-sm font-semibold text-slate-700 mb-2">Type d'acte *</label>
                        <select class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" id="typeActe" name="typeActe" required>
                            <option value="">Sélectionnez un type d'acte</option>
                            <c:forEach var="type" items="${typesActes}">
                                <option value="${type}">${type.libelle} - ${type.coutStandard} DH</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div>
                        <label for="cout" class="block text-sm font-semibold text-slate-700 mb-2">Coût (DH) *</label>
                        <input type="number" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" id="cout" name="cout" step="0.01" min="0" required>
                        <small class="text-slate-500">Modifiez le coût si nécessaire</small>
                    </div>
                </div>
                <div>
                    <label for="description" class="block text-sm font-semibold text-slate-700 mb-2">Notes personnelles</label>
                    <textarea class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" id="description" name="description" rows="3" placeholder="Ajoutez des notes personnelles..."></textarea>
                </div>
                <div class="bg-gradient-to-r from-blue-100/90 to-cyan-100/90 border-l-4 border-blue-500 rounded-xl p-4">
                    <i class="bi bi-info-circle text-blue-700 mr-2"></i>
                    <strong class="text-blue-800">Information :</strong>
                    <span class="text-blue-700"> L'acte sera automatiquement daté et associé à votre nom.</span>
                </div>
            </div>
            <div class="px-6 py-4 bg-slate-50 flex justify-end space-x-3">
                <button type="button" onclick="document.getElementById('ajouterActeModal').classList.remove('show')" class="px-6 py-3 bg-slate-200 text-slate-700 rounded-xl font-semibold hover:bg-slate-300 transition-all">
                    <i class="bi bi-x-lg mr-2"></i>Annuler
                </button>
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl font-semibold hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                    <i class="bi bi-save mr-2"></i>Ajouter l'Acte
                </button>
            </div>
        </form>
    </div>
</div>

<script>
    document.addEventListener('DOMContentLoaded', function() {
        const typeActeSelect = document.getElementById('typeActe');
        const coutInput = document.getElementById('cout');

        if (typeActeSelect && coutInput) {
            typeActeSelect.addEventListener('change', function() {
                const selectedOption = typeActeSelect.options[typeActeSelect.selectedIndex];
                if (selectedOption.value) {
                    const optionText = selectedOption.textContent;
                    const coutMatch = optionText.match(/(\d+(?:\.\d+)?)\s*DH/);
                    if (coutMatch) {
                        coutInput.value = coutMatch[1];
                    }
                }
            });
        }
    });
</script>
</body>
</html>
